...
3. **The distro's D-Bus activation of `org.freedesktop.secrets` must be masked.** This is the subtlest of the three and produces the *same* error message as #2 from a completely different cause. The `gnome-keyring` package ships `/usr/share/dbus-1/services/org.freedesktop.secrets.service` (and `org.gnome.keyring.service`), which map the well-known name `org.freedesktop.secrets` to `gnome-keyring-daemon --start`. The first time anything asks for that name — the first mint-socket connection — the session bus auto-activates a `--start` daemon and gives it the name. Our `mcp-keyring.service` runs a `--unlock` daemon that unlocks a keyring but does **not** wrest a name another daemon already owns, so the unlocked daemon and the name-owning daemon end up being two different processes. Every keystore call routes to the `--start` daemon, whose collection is locked; it tries to raise an interactive `SystemPrompter`, which cannot run headless, and the call fails with the identical `failed to unlock correct collection` message. A desktop never hits this (its keyring owns the name and PAM unlocks it); the *old* dev-infra VM avoided it by giving gnome-keyring a **private** D-Bus bus with none of these activation files on it. On the shared user session bus the installer writes user-level overrides (`~/.local/share/dbus-1/services/{org.freedesktop.secrets,org.gnome.keyring}.service` → `Exec=/bin/false`), kills any `--start` daemon that already grabbed the name (matched by full command line — the process `comm` is truncated to `gnome-keyring-d`, so a bare `pkill gnome-keyring-daemon` matches nothing), and reloads the bus, leaving `mcp-keyring.service` as the only thing that can own the name.
