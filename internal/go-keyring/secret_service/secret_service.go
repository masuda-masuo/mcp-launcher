...
func (s *SecretService) GetLoginCollection() dbus.BusObject {
	// Try the "login" collection first.
	path := dbus.ObjectPath(collectionBasePath + "login")
	if err := s.CheckCollectionPath(path); err == nil {
		obj := s.Object(serviceName, path)
		// If the "login" collection is locked, fall back to the default alias
		// to avoid a dangling unlock prompt (godbus/gnome-keyring compat).
		locked, err := s.isCollectionLocked(obj)
		if err == nil && !locked {
			return obj
		}
	}
	// Fall back to the default alias.
	return s.Object(serviceName, dbus.ObjectPath(loginCollectionAlias))
}

// isCollectionLocked returns true if the collection at *obj* is locked.
func (s *SecretService) isCollectionLocked(obj dbus.BusObject) (bool, error) {
	val, err := obj.GetProperty(collectionInterface + ".Locked")
	if err != nil {
		return false, err
	}
	locked, ok := val.Value().(bool)
	if !ok {
		return false, fmt.Errorf("unexpected Locked property type: %T", val.Value())
	}
	return locked, nil
}
